...
**Correction: the second service is gone, and the ceiling is back to ~$65.** `www-party` was
removed along with the `/party` rewrites — the dinner party is wanted as a mode inside the app
rather than a service beside it, so the container, the image and the route were all scaffolding
for an architecture that is not being built. The two paragraphs above were true while it
existed and are kept for that reason; neither describes the system now. One `--max-instances=1`
service, one instance, ~$65/month as the architectural worst case. The polling analysis still
describes the prototype accurately — it runs on a laptop now, where the cost is nobody's — and
would apply again unchanged the day it is published in any form, which is the reason not to
delete it.
