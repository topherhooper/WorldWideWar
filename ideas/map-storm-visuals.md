...
| 8 | **Design A — the instrument panel.** The doomed wave is a diagonal hatch laid *over* the tiles as an SVG `<pattern>`, so the owner's colour still reads underneath and the storm never competes with ownership for the fill channel. Dead ground keeps its outline in flat ash. The box is three or four lines above the map. **No motion, and no dependence on having seen the previous turn** — everything survives a reload and nothing needs a `prefers-reduced-motion` path. | Rejected: **B, the closing ring** — the doomed frontier stroked as a closed bright loop (`sharedSegments()` already computes those exact segments) and dead ground textured as one continuous scar, plus a turn ruler in the box. It makes the world read as *shrinking* rather than as a set of doomed tiles, and it is the honest picture, since waves really are annuli. Rejected for mark budget: two more vocabularies in an SVG that `tasks/manufactured-map-edges.md` is queued to add to. Rejected: **C, the storm front** — B plus marking the land taken on the most recent resolution distinctly from older dead ground, read from `view.latestReport`'s `storm` event, with a reduced-motion-gated pulse on the warning. This also settles the burn-moment question: **there is no burn moment.** |
...
### The board forgets whose land burned

Surfaced by the question "why does a player keep ownership of a lost territory?" — they
do not. `applyStorm` sets `collapsed = true`, `armies = 0` **and `owner = null`**
(`storm.ts:53-59`), so a collapsed tile has no owner in state at all.

Two consequences worth having written down:

- The hatch is never drawn on lost land. It is drawn on ground you **still own and still
  have armies on**, which is the only reason it changes an order. After the wave lands
  the tile has no player colour to show through anything.
- **"The storm just took three of Sam's provinces" is not renderable after the fact.**
  The prior owner survives nowhere — not in `GameState`, and not in the `storm` event,
  which carries territory ids and an army total and no owners (`types.ts:292`). Any
  design wanting to show *whose* land burned needs an engine change to record it. That
  is a hard limit on design C, not a detail of it.
